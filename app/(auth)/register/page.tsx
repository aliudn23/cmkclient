...
export default function Login() {
    ...
    const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
        ...
        try {
            ...
            if (result.id) 
                alert("Success, your email has been registered.")
                router.replace("/login");
            
        } catch (error: any) {
            console.error(error)
            alert("Failed to register this user, please check your email")
        } finally {
            setLoading(false);
        }
    }
    ...
    return <div className="bg-white rounded-lg border border-gray-300 p-6">
        <div className="xl:w-4xl max-w-md">
            <h1 className="mt-2 text-2xl font-bold text-gray-900 dark:text-white">Register here</h1>
            <p className="mt-1 text-sm text-pretty text-gray-600 dark:text-gray-200">
                Get started by create your account.
            </p>
